...
p_occurence <- ggplot(df_occur_count) + geom_path(mapping = aes(x = Var1, y = Freq)) + xlab("Date") + ylab("Occurance") + ggtitle("Occurence of Crime in 2022") + scale_x_date(date_breaks = "1 month" , date_labels = "%m/%d")
p_occurence
```
...
This plot shows the occurrence of crime at difference times of the day in 2022. The occurrence of crime reaches to its lowest in the morning then gradually goes up. It reaches its peak at 3-5 p.m. then goes down. It's against to our intuitation that most crime occurs at night. In fact, most of the crime occurs in the daytime, especially in the afternoon.

### Law Category Analysis

```{r Occurence_law_2022, message=FALSE}

df_law_22 <- df_ts_22 %>% group_by(law_cat_cd, cmplnt_fr_dt) %>% summarise(n = n()) %>% ungroup()
ggplot(df_law_22, mapping = aes(x = cmplnt_fr_dt, y = n, group = law_cat_cd, color = law_cat_cd)) + geom_line() +labs(x = "Date", y = "Occurence", title = "Occurence of Crime in Different Law Categories in 2022", color = "Law Category")+scale_x_date(date_breaks = "1 month" , date_labels = "%m/%d")
```
...
```{r Occurence_law_hour_2022, message=FALSE}

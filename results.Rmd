...
p_occurence <- ggplot(df_occur_count, mapping = aes(x = Date, y = Frequency)) + geom_path()+ geom_point() + xlab("Date") + ylab("Frequency") + ggtitle("Frequency of Crime in 2022") + scale_x_date(date_breaks = "1 month" , date_labels = "%m/%d")
...
p <- ggplot(df_occur_hour_count, mapping = aes(x = Hour, y = Frequency, group=1))+geom_line() +geom_point()+xlab("Hour") + ylab("Frequency") + ggtitle("Frequency of crime at different times of the day in 2022")
ggplotly(p)
```

This plot shows the frequency of crimes at difference times of the day in 2022. The occurrence of crime reaches to its lowest in the morning then gradually goes up. It reaches its peak at 3-5 p.m. then goes down. It's against to our intuition that most crime occurs at night. In fact, most of the crime occurs in the daytime, especially in the afternoon.

### Law Category Analysis

```{r Occurence_law_2022, message=FALSE, fig.width=10}

df_law_22 <- df_ts_22 %>% group_by(law_cat_cd, cmplnt_fr_dt) %>% summarise(n = n()) %>% ungroup()
colnames(df_law_22) <- c( "Law Category", "Date", "n")
p <- ggplot(df_law_22, mapping = aes(x = Date, y = n, group = `Law Category`, color = `Law Category`))  + geom_line() +geom_point()+labs(x = "Date", y = "Frequency", title = "Frequency of Crime in Different Law Categories in 2022", color = "Law Category")+ scale_x_date(date_breaks = "1 month" , date_labels = "%m/%d")
ggplotly(p)
```

The plot shows the frequency of crimes falling in different law categories in 2022. We can see from the plot the frequency in decreasing number is misdemeanor, felony and violation. They all experience an increase and decrease in trend of occurrence in 2022. Moreover, frequency of misdemeanor has the highest variance while the frequency of violation is the most stable among the frequencies of these three kinds of crimes.

```{r Occurence_law_hour_2022, message=FALSE, fig.width=10}
...
The above graph shows the frequencies of crimes of all three categories reaches their lowest points at 5-6 a.m.. And they reaches their highest points at 3-5 p.m.. We can also see from the plot that in the night, there is another peak for felony and the rate of felony in all categories of crimes goes up. It's to our intuition that the felony often occur at midnight. 
...
This plot shows the frequency of crimes everyday happened in different boroughs in 2022. We can see from the plot that in terms of median, crimes happen in Brooklyn most and happen in Staten Island least everyday. There is few outliers in every box, meaning that the number of crimes everyday happen in a borough is stable.
...
In order to figure out what type of crime that a person in NYC is most likely to encounter, we divided all crime records into 7 types of crimes based on their descriptions: Murder, Assault, Robbery, Sexual Crime, Fraud, Harassment and Others.
...
According to this mosaic plot, we can find that Robbery and Assault are two most frequent crime types occur in general that a person could encounter in NYC. It also tells us many representative characteristics based on classification of victims. For example, for any race and gender, the older a victim was, the more likely he/she got involved in a robbery or theft, which we can intuitively understand the reason since the older people cannot move fast and thus will tend to be targeted by thieves. Besides, we can see from this plot that elderly people will be more easily deceived, resulting in the higher proportion of fraud incidents than younger age groups. In addition, females under 18 might have higher probability to become victims of Sexual Crime than other groups. White and Aisan / Pacific Islanders race have higher probability to encounter Robbery crimes than other race groups.
>>>>>>> 6e83db700328910331a496045eef927c502bd785
